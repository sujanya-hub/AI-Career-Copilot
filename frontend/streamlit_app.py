...
def inject_css():
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .copilot-wordmark { font-family: var(--ff-display); font-size: 1.15rem; font-weight: 700; color: var(--txt); letter-spacing: -0.02em; }
    ...
    .dot-live { width: 5px; height: 5px; border-radius: 50%; background: var(--green); box-shadow: 0 0 6px var(--green); animation: pulse-dot 2s ease infinite; flex-shrink: 0; }
    .dot-offline { width: 5px; height: 5px; border-radius: 50%; background: var(--red); flex-shrink: 0; }
    ...
    .hero-label {
        ...
        font-family: var(--ff-mono); font-size: 0.6rem; letter-spacing: 0.2em; text-transform: uppercase;
        color: var(--blue); background: var(--blue-dim); border: 1px solid rgba(79,138,255,0.18);
        border-radius: 99px; padding: 0.28rem 0.8rem; margin-bottom: 1.25rem;
    }
    .hero-title { font-family: var(--ff-display); font-size: 3.4rem; font-weight: 700; line-height: 1.04; letter-spacing: -0.03em; color: var(--txt); margin: 0 0 1rem; }
    ...
    .hero-sub { font-size: 1rem; color: var(--txt-2); max-width: 560px; line-height: 1.75; font-weight: 400; }
    .hero-stats { display: flex; gap: 2.5rem; margin-top: 2rem; }
    .hero-stat-val { font-family: var(--ff-display); font-size: 1.45rem; font-weight: 700; color: var(--txt); }
    .hero-stat-lbl { font-size: 0.72rem; color: var(--txt-3); margin-top: 0.15rem; letter-spacing: 0.02em; }

    /* INPUT PANEL */
    .input-panel { background: var(--bg-1); border: 1px solid var(--border); border-radius: var(--r-xl); overflow: hidden; transition: border-color 0.3s, box-shadow 0.3s; }
    .input-panel:hover { border-color: var(--border-md); box-shadow: var(--shadow-md); }
    .input-panel-top { display: flex; align-items: center; gap: 0.6rem; padding: 0.85rem 1.2rem; background: var(--bg-2); border-bottom: 1px solid var(--border); }
    .panel-label { font-weight: 600; font-size: 0.82rem; color: var(--txt); }
    .panel-sublabel { font-family: var(--ff-mono); font-size: 0.56rem; letter-spacing: 0.1em; text-transform: uppercase; color: var(--txt-3); margin-top: 0.1rem; }
    ...
    [data-testid="stFileUploaderDropzone"] { background: var(--bg) !important; border: 1.5px dashed var(--border-md) !important; border-radius: var(--r-md) !important; padding: 1.5rem 1rem !important; min-height: 88px !important; transition: border-color 0.2s, background 0.2s !important; }
    [data-testid="stFileUploaderDropzone"]:hover { background: var(--blue-dim) !important; border-color: var(--blue) !important; }
    [data-testid="stFileDropzoneInstructions"] > div > span { font-family: var(--ff-body) !important; font-size: 0.78rem !important; color: var(--txt-3) !important; }
    [data-testid="stFileDropzoneInstructions"] > div > small { font-family: var(--ff-mono) !important; font-size: 0.6rem !important; color: var(--txt-3) !important; }
    [data-testid="stFileUploaderDropzone"] button { background: var(--bg-2) !important; color: var(--blue) !important; border: 1px solid rgba(79,138,255,0.25) !important; border-radius: var(--r-sm) !important; font-family: var(--ff-mono) !important; font-size: 0.65rem !important; padding: 0.28rem 0.8rem !important; box-shadow: none !important; transform: none !important; width: auto !important; }
    ...
    [data-testid="stFileUploaderFileName"] { font-family: var(--ff-mono) !important; font-size: 0.7rem !important; color: var(--green) !important; }
    [data-testid="stFileUploaderFileData"] { font-family: var(--ff-mono) !important; font-size: 0.6rem !important; color: var(--txt-3) !important; }
    ...
    .stTextArea textarea::placeholder { color: var(--txt-3) !important; font-size: 0.8rem !important; }

    /* LIVE EDITOR */
    .live-editor-area textarea {
        background: var(--bg) !important; border: 1.5px solid rgba(79,138,255,0.18) !important;
        border-radius: var(--r-md) !important; color: var(--txt) !important;
        font-family: var(--ff-mono) !important; font-size: 0.76rem !important;
        line-height: 1.85 !important; padding: 1rem !important;
    }
    .live-editor-area textarea:focus { border-color: var(--blue) !important; box-shadow: 0 0 0 3px var(--blue-glow) !important; }

    /* BUTTONS */
    .stButton > button { background: linear-gradient(135deg, #4f8aff 0%, #22d3ee 100%) !important; color: #fff !important; border: none !important; border-radius: var(--r-md) !important; font-family: var(--ff-body) !important; font-size: 0.875rem !important; font-weight: 600 !important; padding: 0.78rem 2rem !important; cursor: pointer !important; transition: all 0.2s !important; box-shadow: 0 4px 20px rgba(79,138,255,0.3) !important; width: 100% !important; letter-spacing: 0.01em; }
    .stButton > button:hover { transform: translateY(-2px) !important; box-shadow: 0 8px 28px rgba(79,138,255,0.45) !important; }
    ...
    /* TABS */
    .stTabs [data-baseweb="tab-list"] { background: var(--bg-1) !important; border: 1px solid var(--border) !important; border-radius: var(--r-md) !important; padding: 0.28rem !important; gap: 0.08rem !important; margin-bottom: 1.75rem !important; }
    .stTabs [data-baseweb="tab"] { background: transparent !important; color: var(--txt-3) !important; border-radius: var(--r-sm) !important; font-family: var(--ff-body) !important; font-size: 0.8rem !important; font-weight: 500 !important; padding: 0.48rem 1rem !important; border: none !important; transition: all 0.15s !important; letter-spacing: 0.01em; }
    ...
    .card-title { font-weight: 600; font-size: 0.85rem; color: var(--txt); display: flex; align-items: center; gap: 0.5rem; margin-bottom: 1.25rem; padding-bottom: 0.8rem; border-bottom: 1px solid var(--border); letter-spacing: 0.01em; }
    ...
    .score-hero::after { content: ''; position: absolute; bottom: -50px; left: 50%; transform: translateX(-50%); width: 200px; height: 100px; background: radial-gradient(ellipse, rgba(79,138,255,0.1), transparent 70%); pointer-events: none; }
    ...
    .score-denom { font-family: var(--ff-mono); font-size: 0.95rem; color: var(--txt-3); }
    .score-tag { display: inline-block; margin-top: 0.75rem; padding: 0.28rem 0.95rem; border-radius: 99px; font-size: 0.76rem; font-weight: 500; letter-spacing: 0.02em; }

    /* IMPROVEMENT TRACKER */
    .improvement-banner { background: var(--bg-1); border: 1px solid var(--border); border-radius: var(--r-lg); padding: 1.1rem 1.5rem; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 1rem; margin-bottom: 1.5rem; position: relative; overflow: hidden; }
    ...
    .score-compare-lbl { font-family: var(--ff-mono); font-size: 0.55rem; letter-spacing: 0.12em; text-transform: uppercase; color: var(--txt-3); margin-top: 0.25rem; }
    .score-compare-arrow { font-size: 1.4rem; color: var(--txt-3); opacity: 0.35; }
    ...
    .delta-msg { font-size: 0.8rem; color: var(--txt-2); line-height: 1.55; max-width: 340px; }
    .delta-msg strong { color: var(--txt); }

    /* PROGRESS */
    .prog-row { background: var(--bg-1); border: 1px solid var(--border); border-radius: var(--r-md); padding: 1.2rem; margin-bottom: 0.6rem; }
    .prog-header { display: flex; justify-content: space-between; margin-bottom: 0.6rem; }
    .prog-name { font-size: 0.8rem; font-weight: 500; color: var(--txt-2); }
    .prog-pct { font-family: var(--ff-mono); font-size: 0.76rem; color: var(--txt); }
    .prog-track { background: var(--bg-3); border-radius: 99px; height: 4px; overflow: hidden; }
    .prog-fill { height: 100%; border-radius: 99px; transition: width 0.8s cubic-bezier(0.16,1,0.3,1); }

    /* METRIC GRID */
    .mgrid { display: grid; grid-template-columns: 1fr 1fr; gap: 0.6rem; margin-top: 0.75rem; }
    .mcard { background: var(--bg-2); border: 1px solid var(--border); border-radius: var(--r-md); padding: 1.1rem 1.2rem; transition: border-color 0.2s; }
    .mcard:hover { border-color: var(--border-md); }
    .mcard-val { font-family: var(--ff-display); font-size: 1.95rem; font-weight: 700; color: var(--txt); line-height: 1; }
    .mcard-lbl { font-family: var(--ff-mono); font-size: 0.56rem; letter-spacing: 0.12em; text-transform: uppercase; color: var(--txt-3); margin-top: 0.35rem; }

    /* KEYWORD PILLS */
    .pill-row { display: flex; flex-wrap: wrap; gap: 0.38rem; }
    .pill { font-family: var(--ff-mono); font-size: 0.67rem; padding: 0.26rem 0.7rem; border-radius: 99px; cursor: default; transition: transform 0.15s, filter 0.15s; white-space: nowrap; }
    ...
    /* KEYWORD SUGGESTION */
    .kw-suggestion-card { background: var(--bg-2); border: 1px solid var(--border); border-left: 2px solid var(--blue); border-radius: 0 var(--r-md) var(--r-md) 0; padding: 1.1rem 1.2rem; margin-top: 0.85rem; animation: slideDown 0.22s cubic-bezier(0.16,1,0.3,1); }
    @keyframes slideDown { from { opacity: 0; transform: translateY(-6px); } to { opacity: 1; transform: translateY(0); } }
    .kw-suggestion-label { font-family: var(--ff-mono); font-size: 0.56rem; letter-spacing: 0.12em; text-transform: uppercase; color: var(--blue); margin-bottom: 0.5rem; font-weight: 500; }
    .kw-suggestion-text { font-size: 0.85rem; color: var(--txt); line-height: 1.7; font-style: italic; margin-bottom: 0.75rem; }
    .kw-placement-chips { display: flex; gap: 0.4rem; flex-wrap: wrap; margin-bottom: 0.75rem; }
    .kw-placement-chip { font-family: var(--ff-mono); font-size: 0.6rem; padding: 0.2rem 0.55rem; border-radius: 99px; background: var(--blue-dim); color: var(--blue); border: 1px solid rgba(79,138,255,0.2); }

    /* LIVE EDIT PANEL */
    .live-edit-panel { background: var(--bg-1); border: 1px solid rgba(79,138,255,0.16); border-radius: var(--r-xl); overflow: hidden; position: relative; }
    .live-edit-panel::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px; background: linear-gradient(90deg, #4f8aff, #22d3ee, #8b5cf6); }
    .live-edit-header { display: flex; align-items: center; justify-content: space-between; padding: 1rem 1.5rem; background: var(--bg-2); border-bottom: 1px solid var(--border); }
    .live-edit-title { display: flex; align-items: center; gap: 0.65rem; font-weight: 600; font-size: 0.875rem; color: var(--txt); }
    .live-pulse { width: 7px; height: 7px; border-radius: 50%; background: var(--blue); box-shadow: 0 0 7px var(--blue); animation: pulse-dot 1.5s ease infinite; }
    .live-edit-meta { font-family: var(--ff-mono); font-size: 0.58rem; color: var(--txt-3); letter-spacing: 0.08em; }

    /* RE-SCORE RESULT */
    .rescore-result { background: var(--bg-2); border: 1px solid var(--border); border-radius: var(--r-lg); padding: 1.25rem 1.5rem; margin-top: 1rem; animation: slideDown 0.28s cubic-bezier(0.16,1,0.3,1); }
    .rescore-scores { display: flex; align-items: center; gap: 1.5rem; margin-bottom: 1rem; }
    ...
    .rescore-score-lbl { font-family: var(--ff-mono); font-size: 0.52rem; letter-spacing: 0.12em; text-transform: uppercase; color: var(--txt-3); margin-top: 0.2rem; }
    .rescore-arrow { font-size: 1.8rem; color: var(--txt-3); opacity: 0.28; }

    /* KEYWORD DIFF */
    .kw-diff-row { display: flex; gap: 1rem; margin-top: 1rem; }
    .kw-diff-col { flex: 1; }
    .kw-diff-title { font-family: var(--ff-mono); font-size: 0.56rem; letter-spacing: 0.12em; text-transform: uppercase; margin-bottom: 0.5rem; font-weight: 500; }
    .kw-diff-title.gained { color: var(--green); }
    .kw-diff-title.lost { color: var(--red); }

    /* LOOP TIMELINE */
    .loop-timeline { background: var(--bg-2); border: 1px solid var(--border); border-radius: var(--r-md); padding: 1rem 1.2rem; margin-top: 1rem; }
    .loop-timeline-title { font-family: var(--ff-mono); font-size: 0.56rem; letter-spacing: 0.12em; text-transform: uppercase; color: var(--txt-3); margin-bottom: 0.75rem; }
    .loop-entry { display: flex; align-items: center; gap: 0.85rem; padding: 0.42rem 0; border-bottom: 1px solid var(--border); }
    .loop-entry:last-child { border-bottom: none; }
    .loop-entry-num { font-family: var(--ff-mono); font-size: 0.58rem; color: var(--txt-3); min-width: 20px; }
    .loop-entry-score { font-family: var(--ff-display); font-size: 0.95rem; font-weight: 700; min-width: 30px; }
    .loop-entry-bar { flex: 1; height: 3px; background: var(--bg-3); border-radius: 99px; overflow: hidden; }
    .loop-entry-fill { height: 100%; border-radius: 99px; }
    .loop-entry-delta { font-family: var(--ff-mono); font-size: 0.62rem; min-width: 50px; text-align: right; }
    .loop-entry-ts { font-family: var(--ff-mono); font-size: 0.52rem; color: var(--txt-3); min-width: 68px; text-align: right; }
    ...
    .decision-badge { display: inline-flex; align-items: center; gap: 0.6rem; padding: 0.55rem 1.2rem; border-radius: 99px; font-weight: 700; font-size: 0.92rem; letter-spacing: 0.01em; }
    .decision-hire { background: rgba(16,185,129,0.1); border: 1.5px solid rgba(16,185,129,0.32); color: #10b981; box-shadow: 0 0 22px rgba(16,185,129,0.12); }
    .decision-maybe { background: rgba(245,158,11,0.1); border: 1.5px solid rgba(245,158,11,0.32); color: #f59e0b; box-shadow: 0 0 22px rgba(245,158,11,0.1); }
    .decision-reject { background: rgba(239,68,68,0.1); border: 1.5px solid rgba(239,68,68,0.32); color: #ef4444; box-shadow: 0 0 22px rgba(239,68,68,0.08); }
    .decision-confidence { font-family: var(--ff-mono); font-size: 0.65rem; color: var(--txt-3); letter-spacing: 0.1em; text-transform: uppercase; margin-top: 0.5rem; }
    .recruiter-10s { background: linear-gradient(135deg, rgba(79,138,255,0.05), rgba(34,211,238,0.03)); border: 1px solid rgba(79,138,255,0.13); border-radius: var(--r-md); padding: 1.1rem 1.2rem; margin-top: 1.2rem; position: relative; }
    .recruiter-10s::before { content: '10-Second Impression'; display: block; font-family: var(--ff-mono); font-size: 0.56rem; letter-spacing: 0.14em; text-transform: uppercase; color: var(--blue); margin-bottom: 0.5rem; font-weight: 500; }
    .recruiter-10s-text { font-size: 0.88rem; color: var(--txt); line-height: 1.75; font-style: italic; }
    .strengths-weaknesses { display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-top: 1.2rem; }
    .sw-col { background: var(--bg-2); border: 1px solid var(--border); border-radius: var(--r-md); padding: 1rem; }
    .sw-col-title { font-family: var(--ff-mono); font-size: 0.58rem; letter-spacing: 0.12em; text-transform: uppercase; margin-bottom: 0.75rem; font-weight: 500; }
    .sw-col-title.green { color: var(--green); }
    .sw-col-title.red { color: var(--red); }
    .sw-item { display: flex; align-items: flex-start; gap: 0.5rem; margin-bottom: 0.5rem; font-size: 0.78rem; color: var(--txt-2); line-height: 1.55; }
    .sw-dot { width: 5px; height: 5px; border-radius: 50%; flex-shrink: 0; margin-top: 0.4rem; }
    .sw-dot.green { background: var(--green); box-shadow: 0 0 5px var(--green); }
    .sw-dot.red { background: var(--red); box-shadow: 0 0 5px var(--red); }
    .risk-indicator { display: inline-flex; align-items: center; gap: 0.5rem; margin-top: 1rem; }
    .risk-label { font-family: var(--ff-mono); font-size: 0.58rem; letter-spacing: 0.1em; text-transform: uppercase; color: var(--txt-3); }
    .risk-badge { font-family: var(--ff-mono); font-size: 0.65rem; padding: 0.22rem 0.7rem; border-radius: 99px; font-weight: 500; }
    ...
    /* SUGGESTIONS */
    .sug-item { background: var(--bg-1); border: 1px solid var(--border); border-radius: var(--r-md); padding: 0.95rem 1.1rem; margin-bottom: 0.55rem; font-size: 0.84rem; color: var(--txt-2); line-height: 1.65; display: flex; align-items: flex-start; gap: 0.85rem; transition: border-color 0.2s, transform 0.18s; }
    .sug-item:hover { border-color: var(--border-md); transform: translateX(2px); }
    .sug-num { font-family: var(--ff-mono); font-size: 0.6rem; color: var(--blue); background: var(--blue-dim); border: 1px solid rgba(79,138,255,0.2); border-radius: 4px; padding: 0.18rem 0.42rem; margin-top: 0.12rem; flex-shrink: 0; min-width: 26px; text-align: center; }

    /* CHAT */
    .chat-wrap { background: var(--bg-1); border: 1px solid var(--border); border-radius: var(--r-xl); overflow: hidden; }
    .chat-messages { padding: 1.2rem; max-height: 380px; overflow-y: auto; display: flex; flex-direction: column; gap: 1rem; }
    .chat-msg-user { align-self: flex-end; background: var(--blue-dim); border: 1px solid rgba(79,138,255,0.2); border-radius: var(--r-md) var(--r-md) 2px var(--r-md); padding: 0.72rem 0.95rem; max-width: 80%; font-size: 0.83rem; color: var(--txt); line-height: 1.6; }
    .chat-msg-ai { align-self: flex-start; background: var(--bg-2); border: 1px solid var(--border); border-radius: 2px var(--r-md) var(--r-md) var(--r-md); padding: 0.72rem 0.95rem; max-width: 85%; font-size: 0.83rem; color: var(--txt-2); line-height: 1.65; }
    .chat-msg-label { font-family: var(--ff-mono); font-size: 0.56rem; letter-spacing: 0.1em; text-transform: uppercase; margin-bottom: 0.35rem; }
    .lbl-user { color: var(--blue); }
    .lbl-ai { color: var(--txt-3); }

    /* ALERTS */
    .alert { border-radius: var(--r-md); padding: 0.82rem 1.05rem; font-size: 0.82rem; margin-bottom: 0.6rem; display: flex; align-items: center; gap: 0.6rem; }
    ...
    /* OPTIMIZER OUTPUT */
    .opt-output { background: var(--bg); border: 1px solid var(--border); border-radius: var(--r-lg); font-family: var(--ff-mono); font-size: 0.75rem; line-height: 1.82; color: var(--txt-2); padding: 1.5rem 1.75rem; max-height: 520px; overflow-y: auto; white-space: pre-wrap; word-break: break-word; }

    /* JOB FIT */
    .fit-row { display: flex; align-items: center; gap: 1rem; background: var(--bg-2); border: 1px solid var(--border); border-radius: var(--r-md); padding: 0.82rem 1.05rem; margin-bottom: 0.5rem; transition: border-color 0.2s, transform 0.18s; }
    .fit-row:hover { border-color: var(--border-md); transform: translateX(2px); }
    .fit-role { flex: 1; font-weight: 500; font-size: 0.85rem; color: var(--txt); }
    .fit-co { font-size: 0.72rem; color: var(--txt-3); margin-top: 0.1rem; }
    .fit-pct { font-family: var(--ff-mono); font-size: 0.78rem; font-weight: 500; padding: 0.22rem 0.65rem; border-radius: 99px; }

    /* INSIGHT ROWS */
    .insight-row { display: flex; align-items: center; justify-content: space-between; padding: 0.68rem 0; border-bottom: 1px solid var(--border); }
    .insight-row:last-child { border-bottom: none; }
    .insight-label { font-size: 0.8rem; color: var(--txt-2); }
    .insight-badge { font-family: var(--ff-mono); font-size: 0.65rem; padding: 0.22rem 0.65rem; border-radius: 99px; font-weight: 500; }

    /* DOWNLOAD BUTTONS */
    [data-testid="stDownloadButton"] button { background: var(--bg-2) !important; color: var(--txt-2) !important; border: 1px solid var(--border-md) !important; border-radius: var(--r-sm) !important; font-family: var(--ff-mono) !important; font-size: 0.68rem !important; letter-spacing: 0.06em !important; padding: 0.42rem 0.95rem !important; transition: all 0.15s !important; box-shadow: none !important; }
    ...
    /* METRIC (native) */
    [data-testid="stMetric"] { background: var(--bg-2); border: 1px solid var(--border); border-radius: var(--r-md); padding: 1rem 1.2rem; }
    [data-testid="stMetric"] label { font-family: var(--ff-mono) !important; font-size: 0.58rem !important; letter-spacing: 0.12em !important; text-transform: uppercase !important; color: var(--txt-3) !important; }
    ...
    [data-testid="stMetricDelta"] { font-family: var(--ff-mono) !important; font-size: 0.7rem !important; }

    /* SPINNER */
    .stSpinner > div { border-top-color: var(--blue) !important; }

    /* STAGED LOADER */
    .loader-step { display: flex; align-items: center; gap: 0.85rem; padding: 0.62rem 0; }
    .loader-step-icon { width: 26px; height: 26px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.7rem; flex-shrink: 0; transition: all 0.3s; font-family: var(--ff-mono); }
    ...
    .loader-step-text { font-size: 0.83rem; text-align: left; }
    ...
    .empty-icon { font-size: 1.5rem; opacity: 0.1; display: block; margin-bottom: 1.25rem; font-family: var(--ff-mono); color: var(--txt-3); }
    .empty-title { font-family: var(--ff-display); font-size: 1.1rem; font-weight: 700; color: var(--txt-3); margin-bottom: 0.5rem; }
    .empty-sub { font-size: 0.8rem; color: var(--txt-3); max-width: 360px; margin: 0 auto; line-height: 1.65; }
    .empty-steps { display: flex; align-items: center; justify-content: center; gap: 1rem; margin-top: 2rem; flex-wrap: wrap; }
    .empty-step { background: var(--bg-1); border: 1px solid var(--border); border-radius: var(--r-md); padding: 0.72rem 1.05rem; font-size: 0.76rem; color: var(--txt-2); display: flex; align-items: center; gap: 0.5rem; }
    .empty-num { font-family: var(--ff-mono); font-size: 0.58rem; color: var(--blue); background: var(--blue-dim); border: 1px solid rgba(79,138,255,0.2); border-radius: 4px; padding: 0.1rem 0.38rem; min-width: 22px; text-align: center; }
    .empty-arrow { color: var(--txt-3); opacity: 0.35; font-size: 0.65rem; }

    /* KEYWORD GROUP */
    .kw-group { background: var(--bg-2); border: 1px solid var(--border); border-radius: var(--r-md); padding: 1rem; margin-bottom: 0.6rem; }
    .kw-group-title { font-family: var(--ff-mono); font-size: 0.6rem; letter-spacing: 0.12em; text-transform: uppercase; color: var(--txt-3); margin-bottom: 0.65rem; }

    /* MISC */
    .divider { border: none; border-top: 1px solid var(--border); margin: 2rem 0; }
    .section-hdr { display: flex; align-items: center; justify-content: space-between; margin-bottom: 1.75rem; }
    .section-title { font-family: var(--ff-display); font-size: 1.4rem; font-weight: 700; letter-spacing: -0.02em; color: var(--txt); }
...
def init_state():
    defaults = {
        ...
        "kw_suggestion_cache":      {},
        "live_resume_text":         None,
        "live_rescore_result":      None,
        "live_rescore_prev_score":  None,
        "score_history":            [],
        "pending_kw_insert":        None,
    }
...
def call_analyze_api(pdf_bytes: bytes, filename: str, jd_text: str) -> dict:
    ...
    resp = requests.post(url, files={"file": (filename, pdf_bytes, "application/pdf")},
                         data={"job_description": jd_text}, timeout=180)
    try:
        raw = resp.json()
    except Exception:
...
def call_optimize_api(resume_text: str, jd_text: str) -> dict:
    ...
    resp = requests.post(url, json={"resume_text": resume_text, "job_description": jd_text}, timeout=180)
    try:
        raw = resp.json()
    except Exception:
        raise ValueError(f"Non-JSON response (HTTP {resp.status_code}): {resp.text[:200]}")
...
def call_rescore_api(resume_text: str, jd_text: str) -> dict:
    ...
    resp = requests.post(url,
                         ...
                         data={"job_description": jd_text}, timeout=120)
    try:
        raw = resp.json()
    except Exception:
        raise ValueError(f"Non-JSON response (HTTP {resp.status_code}): {resp.text[:200]}")
...
def classify_error(exc: Exception) -> str:
    ...
    if "rate_limit" in msg or "429" in msg:
        return "API rate limit reached. Please wait 30 seconds and retry."
    ...
    if "connection" in msg or "connrefused" in msg or "refused" in msg:
        return f"Cannot reach backend at {BACKEND_URL}. Ensure the API server is running."
    if "timeout" in msg:
        return "Request timed out. The backend may be cold-starting — please retry in 20 seconds."
...
def render_alert(msg: str, kind="warn"):
    icon = {"warn": "—", "err": "x", "ok": "+"}.get(kind, "—")
    cls  = {"warn": "alert-warn", "err": "alert-err", "ok": "alert-ok"}.get(kind, "alert-warn")
    st.markdown(f'<div class="alert {cls}"><span style="font-family:var(--ff-mono);font-weight:700;">{icon}</span>{msg}</div>', unsafe_allow_html=True)
...
def group_keywords(keywords: list) -> dict:
    ...
    groups = {"Tools & Technologies": [], "Skills & Methods": [], "Other": []}
    for kw in keywords:
        kl = kw.lower()
        if any(t in kl for t in tools_kw):
            groups["Tools & Technologies"].append(kw)
...
def infer_job_fits(analysis: dict) -> list:
    ...
    role_map = [
        ...
        (["cv","yolo","object detection","segmentation","computer vision"], "Computer Vision Engineer", "Robotics / Automotive"),
...
def advanced_recruiter_decision(analysis: dict, score: int) -> dict:
    ...
    if score >= 78:
        ...
        impression = (f"Resume doesn't clear the bar for this role as written. "
                      ...
                      f"it would likely be filtered before reaching a human reviewer.")
    ...
    if sugs and len(sugs) >= 3: weaknesses.append(f"{len(sugs)} actionable improvement areas identified")
...
def render_improvement_tracker(current_score: int):
    ...
    <div class="improvement-banner {kind}">
        <div style="display:flex;align-items:center;gap:0.75rem;">
            <div style="font-family:var(--ff-mono);font-size:0.55rem;letter-spacing:0.12em;text-transform:uppercase;color:var(--txt-3);">Score History</div>
...
def _record_loop_entry(score: int, prev_score):
...
def _diff_keywords(old_analysis: dict, new_analysis: dict):
    ...
    return sorted(new_matched - old_matched), sorted(old_matched - new_matched)
...
def render_loop_timeline():
    ...
    <div class="loop-timeline">
        <div class="loop-timeline-title">Score Timeline &middot; {len(history)} iterations</div>""",
    unsafe_allow_html=True)
    for entry in history[-6:]:
        ...
        d_str  = f"+{delta}" if delta > 0 else (str(delta) if delta != 0 else "&mdash;")
        ...
        <div class="loop-entry">
            ...
            <div class="loop-entry-bar">
                <div class="loop-entry-fill" style="width:{score}%;background:{bar_c};"></div>
            </div>
...
def render_rescore_result(new_analysis: dict, old_analysis: dict):
    ...
    def _mini_delta(d):
        if d > 0:  return f'<span style="color:var(--green);font-size:0.62rem;">+{d}</span>'
        if d < 0:  return f'<span style="color:var(--red);font-size:0.62rem;">{d}</span>'
        return '<span style="color:var(--txt-3);font-size:0.62rem;">&mdash;</span>'

    gained_pills = "".join(f'<span class="pill pill-green">{kw}</span>' for kw in gained[:12]) if gained else '<span style="color:var(--txt-3);font-size:0.72rem;">None gained</span>'
    lost_pills   = "".join(f'<span class="pill pill-red">{kw}</span>' for kw in lost[:12])   if lost   else '<span style="color:var(--txt-3);font-size:0.72rem;">None lost</span>'
    ...
    <div class="rescore-result">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.65rem;margin-bottom:1.25rem;">
            <div class="mcard" style="padding:0.82rem 1rem;">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <span style="font-size:0.76rem;color:var(--txt-2);">Semantic Similarity</span>
                    {_mini_delta(sem_d)}
                </div>
            ...
            <div class="mcard" style="padding:0.82rem 1rem;">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <span style="font-size:0.76rem;color:var(--txt-2);">Keyword Match</span>
                    {_mini_delta(kws_d)}
                </div>
...
def render_tab_live_loop(analysis: dict, jd_text: str):
    ...
    <div class="live-edit-panel" style="margin-bottom:1.5rem;">
        <div class="live-edit-header">
            ...
            <span class="live-edit-meta">Edit &middot; Re-score &middot; Improve &middot; Repeat</span>
        </div>
    </div>""", unsafe_allow_html=True)

    st.markdown("""
    <div style="background:var(--bg-2);border:1px solid var(--border);border-radius:var(--r-md);
         padding:0.82rem 1.2rem;margin-bottom:1.25rem;
         display:flex;align-items:center;gap:1.5rem;flex-wrap:wrap;">
        <div style="display:flex;align-items:center;gap:0.5rem;font-size:0.78rem;color:var(--txt-2);">
            <span style="font-family:var(--ff-mono);font-size:0.62rem;background:var(--blue-dim);color:var(--blue);
                 border:1px solid rgba(79,138,255,0.2);border-radius:4px;padding:0.1rem 0.42rem;">01</span>
            Edit resume text
        </div>
        <span style="color:var(--border-md);">&#183;</span>
        <div style="display:flex;align-items:center;gap:0.5rem;font-size:0.78rem;color:var(--txt-2);">
            <span style="font-family:var(--ff-mono);font-size:0.62rem;background:var(--blue-dim);color:var(--blue);
                 border:1px solid rgba(79,138,255,0.2);border-radius:4px;padding:0.1rem 0.42rem;">02</span>
            Inject missing keywords
        </div>
        <span style="color:var(--border-md);">&#183;</span>
        <div style="display:flex;align-items:center;gap:0.5rem;font-size:0.78rem;color:var(--txt-2);">
            <span style="font-family:var(--ff-mono);font-size:0.62rem;background:var(--blue-dim);color:var(--blue);
                 border:1px solid rgba(79,138,255,0.2);border-radius:4px;padding:0.1rem 0.42rem;">03</span>
            Re-analyze instantly
        </div>
        <span style="color:var(--border-md);">&#183;</span>
        <div style="display:flex;align-items:center;gap:0.5rem;font-size:0.78rem;color:var(--txt-2);">
            <span style="font-family:var(--ff-mono);font-size:0.62rem;background:var(--blue-dim);color:var(--blue);
                 border:1px solid rgba(79,138,255,0.2);border-radius:4px;padding:0.1rem 0.42rem;">04</span>
            Track score delta
        </div>
    </div>""", unsafe_allow_html=True)
    ...
    with ed_col:
        ...
        if missing:
            ...
            <div style="font-family:var(--ff-mono);font-size:0.58rem;letter-spacing:0.12em;
                ...
                Quick Inject &mdash; click to append keyword sentence
            </div>""", unsafe_allow_html=True)
        ...
        <div style="display:flex;align-items:center;gap:1.25rem;margin-top:0.5rem;
             font-family:var(--ff-mono);font-size:0.6rem;color:var(--txt-3);">
            <span>{wc} words</span>
            <span style="color:{wc_col};">{wc_str} vs original</span>
            <span>{lc} lines</span>
            <span style="margin-left:auto;font-size:0.55rem;opacity:0.5;">Live editor &middot; changes tracked</span>
        </div>""", unsafe_allow_html=True)

    with ctrl_col:
        rescore_res = st.session_state.live_rescore_result
        display_score = _safe_int((rescore_res or analysis).get("ats_score"), 0)
        ...
        <div style="background:var(--bg-1);border:1px solid var(--border);border-radius:var(--r-lg);
            ...
            <div style="font-family:var(--ff-mono);font-size:0.56rem;letter-spacing:0.12em;
            ...
            <div style="font-family:var(--ff-mono);font-size:0.58rem;color:var(--txt-3);margin-top:0.75rem;">
                {loop_count} iteration{"s" if loop_count != 1 else ""} &middot; Baseline: {orig_score}
            </div>
        </div>""", unsafe_allow_html=True)
        ...
        if rescore_disabled and not edited_text:
            st.markdown('<p style="font-size:0.7rem;color:var(--txt-3);text-align:center;margin-top:0.5rem;">Run initial analysis first to populate the editor.</p>', unsafe_allow_html=True)
        ...
        if edited_text and missing:
            ...
            <div style="background:var(--bg-2);border:1px solid var(--border);
                ...
                <div style="font-family:var(--ff-mono);font-size:0.56rem;letter-spacing:0.12em;
                     text-transform:uppercase;color:var(--txt-3);margin-bottom:0.65rem;">Keyword Status</div>
                <div style="margin-bottom:0.5rem;">
                    <div style="font-family:var(--ff-mono);font-size:0.56rem;color:var(--green);margin-bottom:0.35rem;">
                        Present ({len(now_present)})
                    </div>
                    <div class="pill-row">
                        {"".join(f'<span class="pill pill-green">{kw}</span>' for kw in now_present[:10]) or '<span style="color:var(--txt-3);font-size:0.7rem;">None yet</span>'}
                    </div>
                </div>
                <div>
                    <div style="font-family:var(--ff-mono);font-size:0.56rem;color:var(--red);margin-bottom:0.35rem;">
                        Still missing ({len(still_missing)})
                    </div>
                    <div class="pill-row">
                        {"".join(f'<span class="pill pill-red">{kw}</span>' for kw in still_missing[:10]) or '<span style="color:var(--green);font-size:0.7rem;">All covered</span>'}
                    </div>
                </div>
            </div>""", unsafe_allow_html=True)
...
def render_tab_decision(analysis: dict, score: int):
    ...
    with col1:
        ...
        <div class="score-hero card-accent-blue">
            <div style="font-family:var(--ff-mono);font-size:0.58rem;letter-spacing:0.18em;text-transform:uppercase;color:var(--txt-3);margin-bottom:0.35rem;">ATS Score</div>
    ...
    with col2:
        ...
        <div class="decision-card card-accent-green">
            <div style="font-family:var(--ff-mono);font-size:0.58rem;letter-spacing:0.15em;text-transform:uppercase;color:var(--txt-3);margin-bottom:0.85rem;">Recruiter Decision Engine</div>
...
def render_tab_keywords(analysis: dict):
    ...
    with col1:
        ...
        if matched:
            ...
            st.markdown('<p style="color:var(--txt-3);font-size:0.8rem;">No matches found.</p>', unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div class="card" style="border-color:rgba(239,68,68,0.15);">
            ...
            <p style="font-size:0.73rem;color:var(--txt-3);margin-bottom:0.85rem;">
                Click any keyword for a suggested sentence and placement advice.
        ...
        if missing:
            ...
            if sel and sel in missing:
                ...
                <div class="kw-suggestion-card">
                    ...
                    <div class="kw-placement-chips"><span style="font-family:var(--ff-mono);font-size:0.58rem;color:var(--txt-3);margin-right:0.25rem;">Add to:</span>{placement_chips}</div>
                </div>""", unsafe_allow_html=True)
                st.code(sentence, language=None)
                st.markdown('<p style="font-size:0.68rem;color:var(--txt-3);margin-top:-0.5rem;">Copy above &middot; or use Quick Inject in the Live Loop tab</p>', unsafe_allow_html=True)
        else:
            st.markdown('<p style="color:var(--txt-3);font-size:0.8rem;">No critical gaps — well aligned.</p>', unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if jd_top:
        ...
        st.markdown('<p style="color:var(--txt-3);font-size:0.8rem;">No top keywords extracted.</p>', unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)


def render_tab_suggestions(analysis: dict):
    sugs = _safe_list(analysis.get("suggestions"))
    st.markdown('<p style="font-size:0.83rem;color:var(--txt-2);margin-bottom:1.25rem;">AI-generated action items to boost ATS compatibility and recruiter appeal.</p>', unsafe_allow_html=True)
    if sugs:
        ...
        st.markdown('<p style="color:var(--txt-3);font-size:0.83rem;">No suggestions — your resume looks well-aligned for this role.</p>', unsafe_allow_html=True)


def render_tab_job_fit(analysis: dict, score: int):
    st.markdown('<p style="font-size:0.83rem;color:var(--txt-2);margin-bottom:1.25rem;">Based on matched keywords, here are the roles you are most competitive for:</p>', unsafe_allow_html=True)
    ...
    if fits:
        ...
        st.markdown('<p style="color:var(--txt-3);font-size:0.82rem;">Not enough keyword data to infer role fit.</p>', unsafe_allow_html=True)
...
def render_tab_optimizer(analysis: dict, score: int, jd_text: str):
    ...
    <div class="card card-accent-purple" style="margin-bottom:1.5rem;">
        <div style="font-family:var(--ff-display);font-weight:700;font-size:1.15rem;color:var(--txt);margin-bottom:0.4rem;">AI Resume Optimizer</div>
        <div style="font-size:0.83rem;color:var(--txt-2);line-height:1.65;max-width:620px;">The backend AI rewrites your resume tailored to this job description. Keywords integrated naturally, action verbs strengthened, achievements quantified.</div>
    ...
    if st.session_state.is_generating:
        rt = st.session_state.get("resume_text_cache","")
        jd = jd_text.strip() if jd_text else ""
        with st.spinner("Generating optimized resume — 10 to 30 seconds..."):
            try:
                ...
                if result and result.strip():
                    ...
                    st.session_state.opt_error = "Backend returned an empty result. Please retry."
            except Exception as exc:
    ...
    if st.session_state.opt_error:
        ...
        render_alert(f"Resume optimized &middot; {wc} words &middot; {lc} lines &middot; Generated {opt_ts}","ok")
        st.markdown(f"""
        <div style="background:var(--bg-2);border:1px solid var(--border);border-radius:var(--r-md) var(--r-md) 0 0;padding:0.82rem 1.2rem;display:flex;align-items:center;justify-content:space-between;">
            <span style="font-weight:600;font-size:0.85rem;color:var(--txt);display:flex;align-items:center;gap:0.5rem;">
                Optimized Resume <span class="pill pill-green" style="font-size:0.56rem;text-transform:uppercase;letter-spacing:0.08em;">AI-Generated</span>
            </span>
            <span style="font-family:var(--ff-mono);font-size:0.58rem;color:var(--txt-3);">Backend LLM</span>
        </div>
        ...
        with dl1: st.download_button("Download .txt",opt_text,file_name=f"optimized_{ts}.txt",mime="text/plain",use_container_width=True,key="dl_txt")
        with dl2:
            bundle={"generated_at":opt_ts,"original_ats":score,"projected_ats":projected,"optimized_resume":opt_text,"gaps_addressed":missing}
            st.download_button("Download .json",json.dumps(bundle,indent=2),file_name=f"optimized_{ts}.json",mime="application/json",use_container_width=True,key="dl_json")
        with dl3:
            md=f"# Optimized Resume\n_Generated {opt_ts}_\n\n---\n\n{opt_text}"
            st.download_button("Download .md",md,file_name=f"optimized_{ts}.md",mime="text/markdown",use_container_width=True,key="dl_md")
    else:
        st.markdown(f"""
        <div style="background:var(--bg-1);border:1.5px dashed var(--border);border-radius:var(--r-xl);padding:3rem 2rem;text-align:center;margin-top:0.5rem;">
            <div style="font-family:var(--ff-mono);font-size:0.6rem;letter-spacing:0.15em;text-transform:uppercase;color:var(--txt-3);margin-bottom:0.75rem;">Awaiting generation</div>
...
def render_tab_chat(analysis: dict, jd_text: str):
    st.markdown('<p style="font-size:0.83rem;color:var(--txt-2);margin-bottom:1.25rem;">Ask anything about your resume, missing skills, or how to improve for this role.</p>', unsafe_allow_html=True)
    ...
    for i,(col,p) in enumerate(zip(cols,PROMPTS)):
        with col:
            if st.button(p[:28]+"..." if len(p)>28 else p, key=f"qp_{i}", use_container_width=True):
                selected_prompt = p

    chat_html = '<div class="chat-messages">'
    if not st.session_state.chat_history:
        chat_html += '<div class="chat-msg-ai"><div class="chat-msg-label lbl-ai">Career Copilot</div>Resume analyzed. Ask me anything about improving keywords, sections, or positioning for this role.</div>'
    else:
...
def run_analysis_with_stages(pdf_bytes: bytes, filename: str, jd_text: str):
    STAGES = [
        ("01", "Extracting resume data"),
        ("02", "Matching with job description"),
        ("03", "Scoring ATS compatibility"),
        ("04", "Generating intelligence report"),
    ]
    ...
    def render_stages(active: int):
        ...
        for i, (num, label) in enumerate(STAGES):
            ...
            elif i == active: cls, icon_c = "active",  num
            else:             cls, icon_c = "pending", num
        ...
        <div style="background:var(--bg-1);border:1px solid var(--border);border-radius:var(--r-xl);
             padding:2rem 2.5rem;max-width:440px;margin:1.5rem auto;">
            <div style="font-family:var(--ff-mono);font-size:0.58rem;letter-spacing:0.16em;
...
def render_topbar():
    ...
    if ok:
        health_html = '<span class="dot-live"></span>Backend Live'
    else:
        health_html = '<span class="dot-offline"></span><span style="color:var(--red)">Backend Offline</span>'
    st.markdown(f"""
    <div class="copilot-topbar">
        <div class="copilot-brand">
            <div class="copilot-logo">CC</div>
            <span class="copilot-wordmark">Career<em>Copilot</em></span>
        </div>
        <div style="display:flex;align-items:center;gap:0.6rem;">
            <span class="copilot-status">{health_html}</span>
            <span class="copilot-status">AI Resume Intelligence &middot; v2.2</span>
        </div>
    </div>""", unsafe_allow_html=True)
...
def render_hero():
    ...
    <div class="hero-section">
        <div class="hero-label">AI-Powered &middot; TF-IDF + Semantic NLP + LLM</div>
        <h1 class="hero-title">Your AI-powered<br><span>career intelligence</span><br>copilot</h1>
        <p class="hero-sub">Analyze &middot; edit &middot; re-score &middot; repeat. Get your resume to a Strong Match before you apply.</p>
...
def render_input_section():
    ...
    with col_l:
        ...
        <div class="input-panel"><div class="input-panel-top">
            <div><div class="panel-label">Resume</div><div class="panel-sublabel">PDF &middot; Max 10 MB &middot; Text-layer only</div></div>
        ...
        if uploaded_file:
            ...
            st.markdown(f"""
            <div style="display:flex;align-items:center;gap:0.6rem;margin-top:0.6rem;padding:0.5rem 0.82rem;
                 background:var(--green-dim);border:1px solid var(--green-glow);border-radius:var(--r-sm);">
                <span style="color:var(--green);font-family:var(--ff-mono);font-size:0.7rem;">+</span>
                <span style="font-family:var(--ff-mono);font-size:0.7rem;color:var(--green);flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{uploaded_file.name}</span>
                <span style="font-family:var(--ff-mono);font-size:0.6rem;color:var(--txt-3);">{kb} KB</span>
            </div>""", unsafe_allow_html=True)
        st.markdown("</div></div>", unsafe_allow_html=True)
    ...
    with col_r:
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin-top:0.4rem;padding-top:0.4rem;border-top:1px solid var(--border);">
            <span style="font-family:var(--ff-mono);font-size:0.56rem;letter-spacing:0.1em;text-transform:uppercase;color:var(--txt-3);">Plain text &middot; no formatting</span>
            <span style="font-family:var(--ff-mono);font-size:0.6rem;color:{cc};">{n} chars &middot; {cl}</span>
...
def render_export_bar(analysis: dict, score: int, grade_label: str, timestamp: str):
    ...
    <div style="background:var(--bg-1);border:1px solid var(--border);border-radius:var(--r-xl);
        ...
        <div style="width:34px;height:34px;background:var(--blue-dim);border:1px solid rgba(79,138,255,0.2);
             border-radius:8px;display:flex;align-items:center;justify-content:center;
             font-family:var(--ff-mono);font-size:0.75rem;color:var(--blue);">&#8595;</div>
        <div>
            <div style="font-weight:600;font-size:0.85rem;color:var(--txt);">Export Analysis Report</div>
            <div style="font-size:0.7rem;color:var(--txt-3);margin-top:0.1rem;">Full results for sharing or reference</div>
        </div>
    </div>""", unsafe_allow_html=True)
    ts = datetime.datetime.now().strftime("%Y%m%d_%H%M")
    ec1,ec2,_ = st.columns([1,1,4])
    with ec1:
        st.download_button("Download JSON",json.dumps(report,indent=2),file_name=f"copilot_report_{ts}.json",mime="application/json",use_container_width=True)
    with ec2:
        ...
        st.download_button("Download TXT",txt,file_name=f"copilot_report_{ts}.txt",mime="text/plain",use_container_width=True)
...
def main():
    ...
    if analyze_btn:
        ...
        if not jd_text or len(jd_text.strip()) < 50: errors.append("Please paste a job description (minimum 50 characters).")
        if errors:
    ...
    if analysis is not None:
        ...
        <div class="section-hdr">
            ...
            <div style="display:flex;align-items:center;gap:0.6rem;">
                <span class="copilot-status" style="font-size:0.58rem;">Analysis #{count}</span>
                {'<span class="copilot-status" style="font-size:0.58rem;background:var(--blue-dim);border-color:rgba(79,138,255,0.18);color:var(--blue);">' + str(loop_iters) + ' loop iterations</span>' if loop_iters > 1 else ''}
                <span style="font-family:var(--ff-mono);font-size:0.6rem;color:var(--txt-3);">Generated {timestamp}</span>
            </div>
        </div>""", unsafe_allow_html=True)
        ...
        with tab6: render_tab_live_loop(analysis, jd_text)
